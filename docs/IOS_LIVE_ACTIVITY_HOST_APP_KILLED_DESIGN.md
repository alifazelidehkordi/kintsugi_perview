...
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  kintsugi_timer_state (DomainTimerData equivalent)                    │   │
│  │  kintsugi_completed_sessions (array of finished sessions)             │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
                              ▲            ▲
                              │            │
...
struct UserDefaultsKeys {
    static let timerState = "kintsugi_timer_state"
    static let completedSessions = "kintsugi_completed_sessions"
}
...
class IosUserDefaultsTimerStorage(
    ...
    private val userDefaults = NSUserDefaults(suiteName = "group.com.kintsugi.app")
    ...
    fun writePendingAction(action: String) {
        userDefaults?.setObject(action, forKey = "kintsugi_pending_action")
        userDefaults?.synchronize()
    }

    fun readPendingAction(): String? {
        return userDefaults?.stringForKey("kintsugi_pending_action")
    }

    fun clearPendingAction() {
        userDefaults?.removeObjectForKey("kintsugi_pending_action")
    }
}

// UserDefaults keys
object UserDefaultsKeys {
    const val timerState = "kintsugi_timer_state"
    const val completedSessions = "kintsugi_completed_sessions"
}
```
...
@objc public class TimerStateManager: NSObject {
    ...
    private let userDefaults = UserDefaults(suiteName: "group.com.kintsugi.app")
...
**Modified: `KintsugiAppIntents.swift`**

```swift
struct KintsugiTogglePauseIntent: LiveActivityIntent {
        ...
        if state.state == "paused" {
            ...
            if remaining > 0 {
                ...
                let request = UNNotificationRequest(
                    identifier: "kintsugi_timer_finished",
                    content: content,
                    trigger: trigger
                )
    ...
    private func mapToContentState(_ state: TimerState) -> KintsugiActivityAttributes.ContentState {
...
// Similar changes for:
// - KintsugiStopIntent
// - KintsugiAddMinuteIntent
// - KintsugiStartBreakIntent (uses .skip action)
// - KintsugiStartFocusIntent (uses .skip action)
```
...
2. Modify `KintsugiAppIntents.swift` to use `TimerStateManager`
...
### From KintsugiLiveActivityManager (iOS)
...
| `KintsugiAppIntents.swift` | Use `TimerStateManager` instead of NotificationCenter; set label when Live Activity starts |
...
| `KintsugiInProgressLiveActivity.swift` | Update UI after state change from AppIntents |
| `KintsugiLiveActivityManager.kt` (or bridge) | Call `TimerManager.shared.setLabel()` when starting Live Activity |
