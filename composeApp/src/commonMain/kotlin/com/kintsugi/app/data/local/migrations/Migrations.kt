 ...
 */
package com.kintsugi.app.data.local.migrations
        ...
        override fun migrate(connection: SQLiteConnection) {
            ...
            connection.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS localHabit (
                    ...
                    orderIndex INTEGER NOT NULL DEFAULT ${Long.MAX_VALUE},
                    reminderTimeMinutes INTEGER DEFAULT 0,
                    isArchived INTEGER NOT NULL DEFAULT 0,
                    FOREIGN KEY(labelName) REFERENCES localLabel(name)
                    ON UPDATE CASCADE
                    ON DELETE SET DEFAULT
                );
                """.trimIndent(),
            )
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_localHabit_labelName ON localHabit(labelName);")
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_localHabit_isArchived ON localHabit(isArchived);")

            connection.execSQL(
                """
                CREATE TABLE IF NOT EXISTS localHabitStatus (
                    id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                    habitId INTEGER NOT NULL,
                    dateEpochDays INTEGER NOT NULL,
                    FOREIGN KEY(habitId) REFERENCES localHabit(id)
                    ON UPDATE CASCADE
                    ON DELETE CASCADE
                );
                """.trimIndent(),
            )
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_localHabitStatus_habitId ON localHabitStatus(habitId);")
            connection.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS index_localHabitStatus_habitId_dateEpochDays ON localHabitStatus(habitId, dateEpochDays);",
            )
...
val MIGRATION_11_12: Migration =
    object : Migration(11, 12) {
        override fun migrate(connection: SQLiteConnection) {
            connection.execSQL("ALTER TABLE localHabit ADD COLUMN icon TEXT NOT NULL DEFAULT '';")
            connection.execSQL("ALTER TABLE localHabit ADD COLUMN domain TEXT NOT NULL DEFAULT 'جسم';")
            connection.execSQL("ALTER TABLE localHabit ADD COLUMN mve TEXT NOT NULL DEFAULT '';")
            connection.execSQL("ALTER TABLE localHabit ADD COLUMN goal TEXT NOT NULL DEFAULT '';")
            connection.execSQL("ALTER TABLE localHabitStatus ADD COLUMN level INTEGER NOT NULL DEFAULT 2;")
            connection.execSQL(
                """
                CREATE TABLE IF NOT EXISTS localDailyEntry (
                    dateEpochDays INTEGER PRIMARY KEY NOT NULL,
